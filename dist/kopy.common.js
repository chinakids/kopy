...
function ask(prompts = [], mockPrompts = {}) {
  return ctx => __async(function*(){
    ...
    if (mockPrompts && hasMockPrompts) {
      hasMockPromptsAnswers = yield getMockedAnswers(mockPrompts, hasMockPrompts);
    }

    if (noMockPrompts) {
      noMockPromptsAnswers = yield inquirer.prompt(noMockPrompts).then(answers => {
        // prevent from ReferenceErrors
        for (var i = 0, list = noMockPrompts; i < list.length; i += 1) {
          const prompt = list[i];
    ...
    ctx.meta = { answers: Object.assign({}, mockPrompts, hasMockPromptsAnswers, noMockPromptsAnswers) };
  }())
}
